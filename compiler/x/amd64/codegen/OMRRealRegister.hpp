...
class OMR_EXTENSIBLE RealRegister : public OMR::X86::RealRegister
   ...
   static RegMask vectorMaskMask(RegNum idx)
      ...
      switch (idx)
         ...
         default:
            TR_ASSERT_FATAL(idx >= OMR::RealRegister::k1 && idx <= OMR::RealRegister::k7, "vector mask mask valid for k1-k7 only");
            return OMR::RealRegister::noRegMask;
         }
      }
   ...
   */
   ...
   void setMaskRegisterInEvex(uint8_t *evex)
      {
      uint8_t regNum = getRegisterNumber() - k1 + 1;
      *evex = (*evex & 0xf8) | (0x7 & regNum);
      }
