...
//   TR_ASSERT_FATAL(unrollFactor <= 16, "Excessive unrolling detected (unrollFactor > 16)");
   ...
   deps->addPostCondition(loopLimitReg, TR::RealRegister::NoReg, cg);
   begLabel->setStartInternalControlFlow();
   endLabel->setEndInternalControlFlow();

   generateLabelInstruction(TR::InstOpCode::label, node, begLabel, cg);

   generateRegRegInstruction(TR::InstOpCode::MOVRegReg(), node, loopLimitReg, maxIndexReg, cg);
   ...
   if (numElements != 1)
      ...
      generateRegImmInstruction(TR::InstOpCode::ANDRegImms(), node, loopLimitReg, ~(numElements - 1), cg);
      }

   generateRegRegInstruction(TR::InstOpCode::CMPRegReg(), node, indexReg, loopLimitReg, cg);
   ...
   if (numElements == 1)
      {
      generateRegInstruction(TR::InstOpCode::INCReg(), node, indexReg, cg);
      }
   else
      {
      generateRegImmInstruction(TR::InstOpCode::ADDRegImm4(), node, indexReg, numElements, cg);
      }
