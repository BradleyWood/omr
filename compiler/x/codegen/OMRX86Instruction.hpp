...
class X86RegInstruction : public TR::Instruction
   ...
   OMR::X86::Encoding getEncodingMethod()
      {
      OMR::X86::Encoding encoding = TR::Instruction::getEncodingMethod();

      if (encoding == OMR::X86::Default && !self()->cg()->comp()->target().cpu.supportsAVX())
         {
         encoding = OMR::X86::Legacy;
         }
