...
TR::Register* OMR::X86::TreeEvaluator::SIMDreductionEvaluator(TR::Node* node, TR::CodeGenerator* cg)
   ...
   TR::LabelSymbol *reduce64Label = generateLabelSymbol(cg);
   TR::LabelSymbol *doneLabel = generateLabelSymbol(cg);
   reduce64Label->setStartInternalControlFlow();
   doneLabel->setEndInternalControlFlow();
   ...
   switch (vl)
      ...
      case TR::VectorLength128:
         ...
         if (dt.isFloatingPoint())
            ...
            switch (node->getOpCode().getVectorOperation())
               ...
               case TR::vreductionMin:
                  ...
                  generateRegRegImmInstruction(cmpOpcode.getMnemonic(), node, workingReg, workingReg, 0x4, cg,
                                                cmpOpcode.getSIMDEncoding(&cg->comp()->target().cpu, TR::VectorLength128));

                  generateRegRegInstruction(TR::InstOpCode::ORPDRegReg, node, fprReg, fprReg, cg);
                  generateRegRegInstruction(TR::InstOpCode::PTESTRegReg, node, workingReg, fprReg, cg);
                  generateLabelInstruction(TR::InstOpCode::JE4, node, reduce64Label, cg);
         ...
         generateLabelInstruction(TR::InstOpCode::label, node, reduce64Label, cg);
