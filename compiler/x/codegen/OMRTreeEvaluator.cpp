...
OMR::X86::TreeEvaluator::mAnyTrueEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   if (maskReg->getKind() == TR_VMR && cpu->supportsFeature(OMR_FEATURE_X86_AVX512BW))
      ...
      if (cpu->supportsFeature(OMR_FEATURE_X86_AVX512DQ) && maskNode->getOpCode().getVectorDataType().getVectorNumLanes() <= 16)
         {
      ...
      if (cmpMaskNode && !cmpMaskIsConstTrue)
         ...
         TR::InstOpCode andOpcode = TR::InstOpCode::PANDRegReg;
         OMR::X86::Encoding andEncoding = andOpcode.getSIMDEncoding(&cg->comp()->target().cpu, maskType.getVectorLength());

         if (andEncoding != OMR::X86::Legacy)
            {
            generateRegRegRegInstruction(andOpcode.getMnemonic(), node, tmpVRF, maskReg, cmpMaskReg, cg, andEncoding);
            }
         else
            {
            TR::InstOpCode movOpcode = TR::InstOpCode::MOVDQURegReg;
            OMR::X86::Encoding movEncoding = movOpcode.getSIMDEncoding(&cg->comp()->target().cpu, maskType.getVectorLength());
...
OMR::X86::TreeEvaluator::msplatsEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   TR::Register *resultReg = cg->allocateRegister(
      cg->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_AVX512F) ? TR_VMR : TR_VRF
   );

   bool supportsOpMaskDQ = cg->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_AVX512BW);
   bool supportsOpMaskB = cg->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_AVX512DQ);

#ifndef TR_TARGET_64BIT
   TR_ASSERT_FATAL(false, "msplatsEvaluator is only supported on 64-bit");
#endif

   if (resultReg->getKind() == TR_VMR && boolNode->getOpCode().isLoadConst() && boolNode->getInt() == 0)
      {
   ...
   else if (resultReg->getKind() == TR_VMR &&
            boolNode->getOpCode().isLoadConst() &&
            boolNode->getInt() != 0 &&
            (numLanes == 8 && supportsOpMaskB) || (numLanes == 16) || (numLanes > 16 && supportsOpMaskDQ)
            )
      {
      ...
      generateRegRegRegInstruction(maskOpcode.getMnemonic(), node, resultReg, resultReg, resultReg, cg);
      }
   else if (resultReg->getKind() == TR_VMR && boolNode->getOpCode().isLoadConst() && boolNode->getInt() != 0)
      {
      TR_RematerializableTypes rematType = numLanes > 32 ? TR_RematerializableLong : TR_RematerializableInt;
      int64_t mask = numLanes == 64 ? -1 : (1ll << numLanes) - 1;

      // Create constant in GPR because we do not have kxnor opcode for the specific number of lanes
      TR::Register *maskGPR = cg->allocateRegister();
      TR::TreeEvaluator::loadConstant(node, mask, rematType, cg, maskGPR);
      generateRegRegInstruction(InstOpCode::KMOVWMaskReg, node, resultReg, maskGPR, cg);
      ...
      // Handle non-constant input boolean or less than 8 lanes
      ...
      if (resultReg->getKind() == TR_VMR)
         {
         TR::InstOpCode movOpcode = (supportsOpMaskDQ && numLanes > 16)
                                    ? TR::InstOpCode::KMOVQRegMask
                                    : TR::InstOpCode::KMOVWRegMask;
         TR_ASSERT_FATAL((numLanes <= 16 && movOpcode.getMnemonic() == TR::InstOpCode::KMOVWRegMask) || supportsOpMaskDQ, "msplatsEvaluator: kmovq is not supported");
...
TR::Register*
OMR::X86::TreeEvaluator::binaryMaskEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   {
   TR::Node *lhsNode = node->getFirstChild();
   TR::Node *rhsNode = node->getSecondChild();
   TR::Register *lhsReg = cg->evaluate(lhsNode);
   ...
   switch (node->getOpCode().getVectorOperation())
      ...
      case TR::mxor:
         ...
         break;
      default:
         TR_ASSERT_FATAL(false, "Unsupported operation in binaryMaskEvaluator");
         break;
      }

   if (resultReg->getKind() == TR_VMR)
      {
      TR_ASSERT_FATAL(numLanes <= 16 || avx512BW, "binaryMaskEvaluator: Native mask operations on more than 16 lanes requires AVX-512BW");
      ...
      TR_ASSERT_FATAL(encoding != OMR::X86::Bad, "Unsupported instruction binaryMaskEvaluator");

      if (encoding != OMR::X86::Legacy)
         {
         generateRegRegRegInstruction(vectorOpcode.getMnemonic(), node, resultReg, lhsReg, rhsReg, cg);
         }
      else
         {
         generateRegRegInstruction(TR::InstOpCode::MOVDQURegReg, node, resultReg, lhsReg, cg);
         generateRegRegInstruction(vectorOpcode.getMnemonic(), node, resultReg, rhsReg, cg);
         }
      }
...
OMR::X86::TreeEvaluator::vrolEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   TR_ASSERT_FATAL(et != TR::Int8 && !et.isFloatingPoint(), "Vector rotate evaluator does not support 8-bit types nor floating-point types");

   if (cg->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_AVX512F) && et != TR::Int16)
      {
      return TR::TreeEvaluator::vectorBinaryArithmeticEvaluator(node, cg);
      }
   else if (cg->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_AVX2))
      {
      TR_ASSERT_FATAL(vl != TR::VectorLength512 || cg->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_AVX512F) , "512-bit vector rotate not supported without AVX2");
      ...
      TR::InstOpCode::Mnemonic gpr2XMMOpcode = et.isDouble() || et.isInt64() ? TR::InstOpCode::MOVQRegReg8 : TR::InstOpCode::MOVDRegReg4;
      ...
      OMR::X86::Encoding andEncoding = andOpcode.getSIMDEncoding(&cg->comp()->target().cpu, vl);
      OMR::X86::Encoding orEncoding = orOpcode.getSIMDEncoding(&cg->comp()->target().cpu, vl);
      OMR::X86::Encoding shlEncoding = shlOpcode.getSIMDEncoding(&cg->comp()->target().cpu, vl);
      ...
      // Right Shift
      generateRegRegRegInstruction(subOpcode.getMnemonic(), node, vecTmp2Reg, vecZeroReg, vecTmp2Reg, cg, subEncoding);
      ...
      if (maskReg)
         ...
         generateRegRegRegInstruction(orOpcode.getMnemonic(), node, vecTmp2Reg, resultReg, vecTmp2Reg, cg, orEncoding);
