...
class InstOpCode: public OMR::InstOpCode
   ...
   struct OpCode_t
      ...
      template <class TBuffer> typename TBuffer::cursor_t encode(typename TBuffer::cursor_t cursor, uint8_t rexbits, OMR::X86::Encoding encoding) const;
   ...
   // TBuffer should only be one of the two: Estimator when calculating length, and Writer when generating binaries.
   template <class TBuffer> inline typename TBuffer::cursor_t encode(typename TBuffer::cursor_t cursor, uint8_t rex, OMR::X86::Encoding encoding) const
      {
      return isPseudoOp() ? cursor : info().encode<TBuffer>(cursor, rex, encoding);
      }
