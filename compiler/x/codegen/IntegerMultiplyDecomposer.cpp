   ...
   if (decompositionIndex != -1)
      ...
      if (decompositionIndex != -1)
         ...
         if (composition._subsequentShiftTooExpensive == false)
            ...
            TR_ASSERT_FATAL(comp->compileRelocatableCode() || comp->isOutOfProcessCompilation() || comp->compilePortableCode() || comp->target().cpu.is(OMR_PROCESSOR_X86_INTELWESTMERE), "isIntelWestmere failed\n");
            TR_ASSERT_FATAL(comp->compileRelocatableCode() || comp->isOutOfProcessCompilation() || comp->compilePortableCode() || comp->target().cpu.is(OMR_PROCESSOR_X86_INTELSANDYBRIDGE), "isIntelSandyBridge failed\n");
            ...
            TR_ASSERT_FATAL(comp->compileRelocatableCode() || comp->isOutOfProcessCompilation() || comp->compilePortableCode() || comp->target().cpu.is(OMR_PROCESSOR_X86_AMDOPTERON), "isAMDOpteron failed\n");
