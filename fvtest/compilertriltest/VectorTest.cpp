...
TEST_P(ParameterizedVectorTest, VLoadStore) {
    ...
    if (!supportsAVX && vl == TR::VectorLength256) {
        SKIP_ON_X86(UnsupportedFeature) << "Cannot executed 256-bit vector test without AVX hardware";
        SKIP_ON_HAMMER(UnsupportedFeature) << "Cannot executed 256-bit vector test without AVX hardware";
    } else if (!supportsAVX512 && vl == TR::VectorLength512) {
        SKIP_ON_X86(UnsupportedFeature) << "Cannot executed 512-bit vector test without AVX-512 hardware";
        SKIP_ON_HAMMER(UnsupportedFeature) << "Cannot executed 512-bit vector test without AVX-512 hardware";
    }
#endif
    ...
    for (int i = 0; i < TR::DataType::getSize(vt); i++) {
        input[i] = i;
    }

    entry_point(output, input);

    EXPECT_EQ(0, memcmp(input, output, maxVectorLength));
}
